...
import { authenticateWithGoogle, loginWithEmail, useAuth } from "../contexts/AuthContext";
...
export default function LogInPage() {
  ...
  const { user, userData, LoggedInGuard } = useAuth();
  ...
  async function handleEmailLogin(e: React.FormEvent<HTMLFormElement>){
    e.preventDefault();
    if (!check(email, InputFieldCheckType.EMAIl) && !check(password, InputFieldCheckType.PASSWORD)) return;
    await loginWithEmail(email, password);
    LoggedInGuard!("signup");
  }
                ...
                >
                  Login
								...
								<Box sx={{
									...
									<Typography variant="caption" fontWeight={"bold"} component="span">OR</Typography>
								</Box>
                <Box
                  sx={{
                    width: "calc(100% - 60px)",
